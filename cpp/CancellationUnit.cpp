...
CancellationUnit::CancellationUnit(TimeWarpSimulation *simulation)
{
...
bool CancellationUnit::rollback(ap_uint<16> lp_id, ap_int<32> to_time)
{
    ...
    while (current != 0xFFFF && buffer[current].event.send_time > to_time)
    {
        ...
        if (simulation->route_event(anti_message))
        {
...
TimeWarpSimulation g_simulation;
...
void cancellation_unit_kernel(ap_uint<2> op, TimeWarpEvent event, TimeWarpEvent &result, bool &success)
{
    ...
    static CancellationUnit cancellation_unit(&g_simulation);
    ...
    switch(op)
    {
        ...
        case 1:  // Rollback operation
            success = cancellation_unit.rollback(event.sender_id, event.send_time);
            result = event;
            break;
